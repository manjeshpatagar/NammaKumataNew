...
import { ChevronRightIcon, BanknotesIcon, CreditCardIcon, BuildingLibraryIcon, CurrencyRupeeIcon, StarIcon, ArrowLeftIcon } from '@heroicons/react/24/outline'
import { useRouter } from 'next/navigation'
import Link from 'next/link'

const subcategories = [
  { name: 'Nationalized Banks', icon: <BuildingLibraryIcon className="h-7 w-7 text-blue-600 mr-4" />, link: '/category/banks/nationalized-banks' },
  { name: 'Private Banks', icon: <BanknotesIcon className="h-7 w-7 text-green-600 mr-4" />, link: '/category/banks/private-banks' },
  { name: 'Co-operative Banks', icon: <CurrencyRupeeIcon className="h-7 w-7 text-yellow-600 mr-4" />, link: '/category/banks/co-operative-banks' },
  { name: 'ATMs', icon: <CreditCardIcon className="h-7 w-7 text-purple-600 mr-4" />, link: '/category/banks/atms' },
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="sticky top-0 z-10 bg-white border-b p-4 flex items-center shadow-sm">
        ...
        <h1 className="text-xl font-extrabold text-black flex-1 text-center tracking-wide">Banks</h1>
      </div>
